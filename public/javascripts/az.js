	...
	$.ajax({
	        ...
	        success: function(data) {
	            ...
	            for(var i=0;i<data.length;i++){
	            	html+='<tr><td>'+data[i].username+'</td><td>'+data[i].uid+'</td><td>'+data[i].password+'</td><td>'+data[i].tel+'</td><td>'+data[i].regtime+'</td><td id = '+data[i].uid+'><button id="btn">修改</button></td></tr>'
	            }
	        	$('.zhj').append(html)
	        }
